...
class Service {
  ...
  async createinterview(
    ...
    const existingscheduledinterview =
      await this.repository.checkInterviewOfStatus(userid, "scheduled");

    const existingrunninginterview =
      await this.repository.checkInterviewOfStatus(userid, "running");
  ...
  async getAllCompletedInterviews(userid) {
    ...
    const completedInterviews = await this.repository.getCompletedInterviews(
      userid
    );
  ...
  async calculateRank(city, country, finalScore) {
    ...
    const countryRank = await this.repository.getCountryRank(
      country,
      finalScore
    );
